                ...
                <v-card-text>
                    <v-data-table :headers="headers" :items="items" :search="search" dense :items-per-page="15">
                        ...
                        <template v-slot:footer.prepend>
                            <v-btn text @click="dialogs.indicator_sgic_form = true">New Satisfaction Indicators</v-btn>
                        </template>
                    </v-data-table>
                </v-card-text>
            </v-card>
...
export default {
    ...
    methods: {
        ...
        manage(){
            this.loading = true;
            window.location.href = route('indicators.satisfaction.manage', {
                satisfactionId: 1
            })
        }
    },
